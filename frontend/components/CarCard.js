    ...
    <div className="card p-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-grow min-w-0">
          <h3 className="text-lg font-semibold mb-1">{brand} {model}</h3>
          <p className="text-gray-500 text-sm">{year} | {license_plate || 'Номер не указан'} | Пробег: {mileage || 0} км</p>
          
          <div className="mt-3 space-y-1">
            <p className="text-sm text-gray-600">Посл. ТО: {last_service_mileage || 0} км</p>
            <p className="text-sm text-gray-600">Интервал: {service_interval_km || 10000} км</p>
            {needsService ? (
              <p className="text-sm text-red-600 font-semibold mt-1 bg-red-50 inline-block px-3 py-1 rounded-lg">Требуется ТО!</p>
            ) : (
              <p className="text-sm text-green-600 bg-green-50 inline-block px-3 py-1 rounded-lg mt-1">След. ТО через {remainingKm} км</p>
            )}
          </div>
        </div>
