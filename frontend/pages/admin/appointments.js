...
export default function AdminAppointments() {
  ...
  if (loading) return <div className="text-center py-24 text-steel-400">Загрузка записей...</div>;
      ...
      <div className="max-w-6xl mx-auto mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <input id="search-input" type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Введите ID или статус..." className="input-dark" />
          </div>
          <div className="flex gap-4">
            <div className="flex-1">
              <label htmlFor="date-filter" className="input-label">Фильтр по дате</label>
              <input id="date-filter" type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="input-dark" />
            </div>
            <div className="flex-1">
              <label htmlFor="plate-filter" className="input-label">Поиск по номеру</label>
              <input id="plate-filter" type="text" value={plateQuery} onChange={(e) => setPlateQuery(e.target.value)} placeholder="Гос. номер..." className="input-dark" />
            </div>
          </div>
        </div>
