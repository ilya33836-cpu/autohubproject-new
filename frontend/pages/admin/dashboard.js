...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="text-center py-24 text-steel-400">Загрузка админ-панели...</div>;
  if (error) return <div className="text-center py-24"><p className="text-red-500 mb-2">{error}</p><Link href="/" className="text-steel-600 hover:text-steel-700">Вернуться на главную</Link></div>;
  ...
  const statCards = [
    { label: 'Новые заявки', value: stats.new_appointments_count, color: 'text-steel-600', bg: 'bg-steel-50' },
  ...
  return (
    <AdminLayout>
      <div className="max-w-6xl mx-auto mt-8">
        <div className="mb-10">
          <p className="text-steel-600 font-semibold tracking-widest uppercase text-sm mb-2">Админ-панель</p>
          <h1 className="text-3xl md:text-4xl font-extrabold">Добро пожаловать, {currentUser.full_name || currentUser.username}</h1>
          <p className="text-steel-400 mt-1">Роль: {currentUser.role}</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            {statCards.map((s) => (
              <div key={s.label} className="card p-6">
                ...
                <h3 className="text-sm font-medium text-steel-400">{s.label}</h3>
          ...
          {navCards.map(c => (
            <Link key={c.href} href={c.href} className="card p-6 group cursor-pointer">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold mb-1 group-hover:text-steel-600 transition-colors">{c.title}</h2>
                  <p className="text-steel-400">{c.desc}</p>
                </div>
                <div className="w-10 h-10 rounded-xl bg-steel-50 flex items-center justify-center group-hover:bg-steel-100 transition-colors">
                  <svg className="w-5 h-5 text-steel-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </div>
              </div>
            </Link>
          ))}
